...
PROCESSED_STATUSES = ("fetched", "extracted", "summarised", "embedded", "done", "error")
...
CREATE TABLE IF NOT EXISTS articles (
    ...
    embedding       BLOB,           -- nomic-embed-text vector, packed (e.g. via numpy.tobytes())
    cluster_id      INTEGER REFERENCES clusters(id),
    processed_status TEXT NOT NULL DEFAULT 'fetched'
                    CHECK (processed_status IN ('fetched','extracted','summarised','embedded','done','error'))
);

-- Groups of articles covering the same story, assigned during a later
-- clustering phase (via embedding similarity).
